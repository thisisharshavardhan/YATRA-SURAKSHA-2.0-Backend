...
/**
 * @desc    Get Gemini AI assistant context for current user
 * @route   GET /api/gemini-assistant/context
 * @access  Private
 */
export const getAssistantContext = asyncHandler(async (req, res) => {
    ...
    const lastLocation = await geminiRealtimeService.getLastLocation(user._id);

    // Get nearby safety info if location exists
    let safetyInfo = null;
    if (lastLocation) {
        safetyInfo = await geminiRealtimeService.getNearbySafetyInfo(
    ...
    // Get active trips
    const activeTrips = await geminiRealtimeService.getActiveTrips(user._id);

    // Get recent alerts
    const recentAlerts = await geminiRealtimeService.getRecentAlerts(user._id);
 ...
 * @route   GET /api/gemini-assistant/safety-info
 * @access  Private
 */
export const getLocationSafetyInfo = asyncHandler(async (req, res) => {
    ...
    const safetyInfo = await geminiRealtimeService.getNearbySafetyInfo(lng, lat);
...
/**
 * @desc    Send text message to Gemini AI assistant (REST endpoint for non-realtime use)
 * @route   POST /api/gemini-assistant/chat
 * @access  Private
 */
export const chatWithAssistant = asyncHandler(async (req, res) => {
    ...
    if (longitude && latitude) {
        ...
        currentLocation = await geminiRealtimeService.getLastLocation(user._id);
    }

    // Build the context for a simple text response
    const safetyInfo = currentLocation
        ? await geminiRealtimeService.getNearbySafetyInfo(currentLocation.longitude, currentLocation.latitude)
        : null;
    ...
    res.status(200).json({
        ...
        websocketEvents: {
            ...
            sendAudio: 'gemini:audio',
            sendText: 'gemini:text',
...
/**
 * @desc    Get Gemini session status
 * @route   GET /api/gemini-assistant/session
 * @access  Private
 */
 ...
 * @route   GET /api/gemini-assistant/emergency-info
 * @access  Private
 */
 ...
 * @route   GET /api/gemini-assistant/travel-tips
 * @access  Private
 */
